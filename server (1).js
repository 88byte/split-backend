...
app.post('/submit-score', function(req, res) {
  ...
  supabase.from('scores').insert({ username, score, run_time_ms, level })
    .then(function() {
      return supabase.from('game_sessions').insert({ username, score, run_time_ms, level });
    })
    .then(function() { res.json({ ok: true }); })
    .catch(function(e) {
...
app.get('/rank/:username', function(req, res) {
  supabase.from('leaderboard').select('score').eq('username', req.params.username).single()
    .then(function(result) {
      if(!result.data) return res.json({ rank: null });
      return supabase.from('leaderboard')
        .select('*', { count: 'exact', head: true })
        .gt('score', result.data.score)
        .then(function(r) { res.json({ rank: (r.count || 0) + 1, score: result.data.score }); });
    })
...
app.post('/stripe-webhook', function(req, res) {
  ...
  if(event.type === 'checkout.session.completed') {
    ...
    supabase.from('purchases').insert({
      ...
      return supabase.from('users').upsert({ username, email, tier }, { onConflict: 'username' });
    }).then(function() {
    ...
    .then(function(statsResult) {
      return supabase.from('purchases')
        ...
        .then(function(purchResult) {
          res.json({ stats: statsResult.data, recent_purchases: purchResult.data });
        });
    })
    .catch(function(e) {
